...
final public class LocalNetworkActorSystem: DistributedActorSystem,
    ...
    let nodeName: String
    let serviceName: String = "_rayriffy._jacquard._tcp"
    ...
    private func startNetworking() {
        ...
        browser.start { result in
            ...
            let endpointConnection = NWConnection(to: result.endpoint, using: parameters)
            // -----

            let connection = Connection(connection: endpointConnection, deliverMessage: { data, nwMessage in
                self.decodeAndDeliver(data: data, nwMessage: nwMessage, from: endpointConnection)
            })
